\input{settings/settings}

\usepackage{lipsum}

%\usepackage{ragged2e}
\usepackage{booktabs, tabularx}

\begin{document}
	
	%\pagenumbering{roman}

   %\begin{titlepage}
	%	\include{chapters/titlepage}
   %\end{titlepage}

   %\tableofcontents

   %\cleardoublepage
   \pagenumbering{arabic}
   \title{The Research Plan}
   \author{Philipp Mitterbach}
   \maketitle

   \vspace{5cm}
   %The research plan should describe the following items for assessment:
   %\begin{itemize}
   %\item The subject or title of your research project;
   %\item Your approach to the project;
   %\item Your proposed plan of approach for the continuation of your research for the following 12 months
   %\end{itemize}

   %Your research plan should also include:
   %\begin{itemize}
   %\item A brief summary of the most relevant literature used;
   %\item If applicable, publications or planned publications;
   %\item A list of presentations or lectures given in your first year of PhD research and those you plan to give.
   %\end{itemize}

   \include{chapters/researchPlan}
   %\include{chapters/trainingPlan}
   %\include{chapters/additionalPoints}

   \bibliographystyle{acm}
	 \addcontentsline{toc}{chapter}{\bibname}
   \bibliography{literature/fyrep_literature.bib}

\end{document}
